...
This guide is to help users debug applications that are deployed into Kubernetes and not behaving correctly. This is _not_ a guide for people who want to debug their cluster. For that you should check out the cluster troubleshooting guide.
...
* **You don't have enough resources**: You may have exhausted the supply of CPU or Memory in your cluster, in this case you need to delete Pods, adjust resource requests, or add new nodes to your cluster. See the Compute Resources document for more information.
...
See the Debug Services document for more information.

## What's next

If none of the above solves your problem, follow the instructions in the Debugging Service document to make sure that your `Service` is running, has `Endpoints`, and your `Pods` are actually serving; you have DNS working, iptables rules installed, and kube-proxy does not seem to be misbehaving.
